...
describe('Blob metadata under concurrent derivative uploads', () => {
  beforeAll(() =>
    vi.stubGlobal('fetch', () => {
      throw new Error('Network is disabled in adapter unit tests')
    }),
  )
  ...
  const upload = (data: Record<string, unknown>, filename: string) =>
  ...
  it('keeps a client-uploaded original while recording every randomized size filename', async () => {
    const data = {
      filename: 'photo-client-unique.jpg',
      sizes: {
  ...
  it('keeps the original and derivatives distinct for server-side uploads too', async () => {
    const data = {
      ...
      sizes: { thumbnail: { filename: 'converted-300x225.jpg' } },
    }
  ...
  it('uses direct thumbnail URLs and falls back for images smaller than the thumbnail size', () => {
    expect(
      adminThumbnail({ doc: { sizes: { thumbnail: { url: 'https://store.example/thumb.jpg' } } } }),
    ).toBe('https://store.example/thumb.jpg')
    expect(
      adminThumbnail({ doc: { mimeType: 'image/jpeg', url: 'https://store.example/small.jpg' } }),
    ).toBe('https://store.example/small.jpg')
