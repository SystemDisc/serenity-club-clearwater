...
if (!manifestPath)
  throw new Error('Use --manifest path.json; this is a dry run unless --apply is supplied')
if (apply && !backupPath) throw new Error('--apply requires --backup path.json')
if (
  apply &&
  !isLocalDatabase(process.env.DATABASE_URL) &&
  (!process.env.REVALIDATE_URL || !process.env.CRON_SECRET)
) {
  throw new Error(
    'Production repair requires REVALIDATE_URL and CRON_SECRET for final cache invalidation',
  )
}
...
if (manifest.unresolved?.length)
  throw new Error('Resolve every ambiguous file before repairing media')
...
try {
  ...
  for (const repair of manifest.repairs) {
    ...
    for (const change of [repair.changes.original, ...Object.values(repair.changes.sizes)]) {
      ...
      if (!response.ok)
        throw new Error(`Media ${repair.id}: replacement file returned ${response.status}`)
    }
    prepared.push({
      repair,
      data,
      before: { id: doc.id, filename: doc.filename, sizes: doc.sizes, updatedAt: doc.updatedAt },
    })
  }
  console.log(
    JSON.stringify({
      mode: apply ? 'apply' : 'dry-run',
      checked: prepared.length,
      changes: prepared.filter(({ data }) => Object.keys(data).length).length,
    }),
  )
  if (apply) {
    await writeFile(
    ...
    try {
      for (const { repair, data, before } of prepared) {
        ...
        if (current.updatedAt !== before.updatedAt)
          throw new Error(`Media ${repair.id} changed during validation; rerun the dry run`)
        ...
        if (Object.keys(prepareMediaRepair(verified, repair)).length)
          throw new Error(`Media ${repair.id} repair did not persist`)
        console.log(`Repaired media ${repair.id}`)
      }
    } finally {
      if (repaired && process.env.REVALIDATE_URL) {
        const result = await fetch(process.env.REVALIDATE_URL, {
          ...
          body: JSON.stringify({ collections: ['media'] }),
          signal: AbortSignal.timeout(30_000),
        })
        if (!result.ok)
          throw new Error(
            `Metadata repaired, but public invalidation failed (${result.status}); retry invalidation before marking the repair complete`,
          )
      }
    }
  }
...
console.log('Media repair verification complete')
process.exit(0)
