...
Open `/fundraiser-display` in a full-screen browser on the clubhouse display. The QR code leads directly to the published Zeffy capital campaign. The optional on-screen checkout opens Zeffy's own page in a separate frame; card details are never handled by this application.
