...
Prepared September 15, 2026; expanded after the final public-site/source pass and the user's dues, meeting-format, and DOCX clarifications. **Core implementation merged and deployed; September content published and key live admin workflows verified.** This plan follows the [full admin UX audit](../audits/2026-09-15-admin-ux.md). Finding IDs A01–A27 refer to that report. The design below remains the target; the implementation status distinguishes tested work from remaining work.
...
- Committed workspace foundation (`9985cc0`): task dashboard, section directory/help, restricted manager tools, readable forms, larger selected-image previews, and a paginated visual library picker. A production-mode local build passed; browser checks passed for save/reload of a selected image, Escape/focus return, 320px layout, and anonymous access restrictions. The fifth album task and monthly readiness were completed in the later implementation below.
- Structured meetings were committed in `ca30472`: additive sessions/exceptions, weekday checkboxes, date preview, safe legacy backfill, and a shared public occurrence resolver. The local migration and seven date-rule tests plus an integration test passed. See the [11-record migration review](../audits/2026-09-15-meeting-migration-review.md). The weekly overview/editor is now implemented below. Actual format/attendance confirmation remains outstanding.
...
- Meeting workflow polish adds a searchable dated agenda, group-list alternative, same-room/start-time warnings, and responsive weekday cards. “Separate this day” preserves monthly variations and dated exceptions while allowing a single day’s settings to differ. Unit tests cover six weeks of equivalent resolved dates; a production-mode browser test verifies separating Monday, saving a different format, retaining Tuesday, finding the group, and 320px reflow. Legacy schedule evidence is read-only.
- Albums now have draft/versioned records, covers, dates, stable addresses, paginated public pages, sitemap entries, and a dashboard task. The main gallery shows ungrouped photos and album covers. Anonymous collection reads also enforce the album’s publication state. Integration and production-mode browser tests passed for publication, unpublication, preserving membership, main-gallery separation, and protecting nonempty albums from permanent deletion. The bulk-upload workflow and public viewer polish were completed in the later implementation below.
- Recovery now uses Payload Trash for public content, with permanent deletion restricted to managers. Shared files are protected from deletion/replacement while referenced by saved content, trash, or retained versions; the media editor provides an authenticated usage report. Anonymous reads cannot opt into trashed content. Schema-walker unit tests, access/history integration tests, and a production-mode browser test passed for trash, intact file retrieval, blocked deletion, restore, and public freshness.
- The batch service (`4a5fce6`) and visual Add photos workspace now retain drafts, hash original images, bound uploads to two at a time, offer library multi-selection, captions, cover/order controls, exclusion, retry, and explicit partial publication. Transaction/access tests passed for stale edits, rollback, duplicate selection, and repeated publication. Production-mode browser workloads passed for 30 ungrouped and 50 album photos, including an interrupted upload, refresh/reselection, no duplicate placements, independent public visibility, pagination, and 320px reflow. The local Chrome screen was also visually inspected. The native Blob client-upload branch still needs deployment verification; local tests exercise multipart uploads.
- Website details now has page-oriented tabs for shared contact/hours, homepage pictures, About, facilities, and giving/sponsorship wording. Maps derive from the same address and have an editor preview link; social links have a public consumer. Menus offer named built-in destinations, manager access, and retained history. Shop/team/rules use clearer labels; new shop URLs generate automatically while existing URLs are frozen during migration. Integration tests verify editor/manager boundaries and persisted version restoration; a production-mode browser test verified repeated contact-hour changes and restoration becoming public without rebuilding. Payload global restoration bypasses field access, so restoring the whole settings record is explicitly manager-only.
- The public gallery now has an enlarged viewer with keyboard arrows, Escape/focus return, touch swiping, captions, and an original-file link. Only the selected photo loads in the enlarged view. The production-mode keyboard/swipe/320px regression and automated accessibility scan passed; desktop Chrome visual inspection confirmed the image and controls. Human screen-reader and phone-device checks remain release acceptance work.
- News & updates now has a curated writing toolbar, optional cover/byline/introduction, and an article-photo editor with captions and descriptions. Opening Create no longer creates empty drafts; meaningful first drafts are explicit and subsequent edits autosave. Public `/posts` addresses remain stable, with a paginated index, canonical metadata, sitemap/navigation, authenticated drafts, and publication invalidation. The production-mode browser regression passed for pasted headings/lists, cover and body photos, caption/description retention, fast photo saves, 320px layout, draft isolation, authenticated preview, publication without rebuilding, autosaved revisions, and version restoration. A supported nested-form readiness check prevents a fast Save from racing Payload's block validation. News navigation backfill is idempotent and dry-run by default.
- Batch refinements now recognize the same image selected through a file and the library, append to existing album order, allow older albums' direct Add photos links, support GIF covers consistently, and offer drag sorting alongside move buttons. Four batch integration tests passed, including mixed-selection deduplication and preserved positions.
- The remaining everyday workspace now has meaningful drafts, monthly readiness, resumable batches, current/proposed flyer previews, event/date previews, private autosave after the first explicit draft, publication status, meeting-check recording, and five short task guides. Password recovery has a named action and neutral account-existence wording. Existing photos can be moved, reordered, assigned as covers, or sent to Trash without deleting shared files.
- Local release checks passed: 48 unit tests, 15 integration tests, 29 browser scenarios, lint with zero warnings, type checking, and the production build. A fresh production-copy migration/backfill rehearsal preserved all 93 original records across seven content tables. See the [release record](../operations/2026-09-16-admin-release.md) for evidence and live rollout status.
- The core release is deployed. All 44 production thumbnails and the six core public routes passed live checks; private photo batches reject anonymous access. Production Word conversion/publication, original retention, photo organization/album publication, dues editing, and all September updates are now verified. Volunteer/device acceptance remains. The optional AI pilot still needs a provider, data-handling decision, cost limit, and runner; manual monthly publishing must not depend on that decision.
...
Per the user’s September 16 clarification, useful earlier descriptions remain visible while formats are **Needs confirmation**, accompanied by a short public reminder. Day-specific formats explicitly listed on the supplied old club site are transferred with their provenance retained; they are not marked as freshly checked with organizers. Do not invent missing formats or claim both formats on a day whose individual format is known.
...
The recommended first implementation package is phases 0 and 1: correct the meeting logic, identify unverified format claims, expose the dues reminder under its real purpose, make the important tasks easy to find, make pictures large enough to recognize, and clarify what is saved versus public. Then deliver structured meetings/monthly updates and bulk photos/albums in the agreed order. The user subsequently authorized implementation. Core phases 0–5 are now deployed; this does not authorize guessed schedule corrections. The implementation status and release record distinguish completed checks from pending production workflows and human acceptance.
...
The final [GitHub verification run](https://github.com/SystemDisc/serenity-club-clearwater/actions/runs/35132756157) passed on a clean Linux runner for `bc6aa2b`, including all 30 browser scenarios. The user subsequently restored the admin login and authorized merging/deployment. September content is published; real conversion, saved/public state, bowling organization, and dues editing passed in production. See the release report for follow-up fixes and remaining human acceptance.


### September 16 content and meeting follow-up
