...
Scheduled Pages/Posts publishing is run by Vercel Cron once per minute at `/api/payload-jobs/run`, authenticated with `CRON_SECRET`. Each call processes up to ten due jobs. The same request scope invalidates public content after jobs complete. Editors can schedule content; only administrators and the cron secret can execute the runner. Inspect failed jobs in Payload and retry after correcting the cause; do not repeatedly create duplicate schedules. See [Payload's serverless queue documentation](https://payloadcms.com/docs/jobs-queue/queues).
