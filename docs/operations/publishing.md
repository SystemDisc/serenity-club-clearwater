...
Tests enable scheduling and exercise authenticated publish/unpublish jobs. Run `npm run generate:types` after changing fields; the script enables `ENABLE_SCHEDULED_PUBLISHING=true` so the committed types consistently include the optional task. Automatic type generation is disabled to prevent a development server with scheduling disabled from removing those types. The migration history also retains that schema when scheduling is disabled.
