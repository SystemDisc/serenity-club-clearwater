...
## Homepage flyer as the first event card

Further user feedback rejected the standalone centered flyer because it still left large unused space. The homepage now places the monthly flyer first in the same three-column grid as upcoming events: one flyer and two event cards, or three events when no current flyer exists. Its border, padding, and uncropped 4:3 image area match the event cards. Visitors can open the original flyer or read the text on Events. The full Events-page presentation remains available. Local Chrome verified the real September flyer and event image samples at desktop, 390px, and 320px widths, including loaded images and no horizontal overflow. Targeted lint and the production build with TypeScript passed. Fixtures were confined to the isolated test database.
