...
Limits: 25 MiB input download, 15-second download timeout, 30 seconds per conversion subprocess, at most 25 PDF pages, only the first page rasterized, maximum 4000-pixel long edge and 16 million decoded pixels. The caller caps output at 20 MiB and waits at most two minutes. Each service instance converts one document at a time; busy requests receive 429 and `Retry-After: 10` rather than an unbounded queue. Invalid JSON and oversized requests fail clearly.
